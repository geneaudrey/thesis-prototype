        ...
        <div class="row m-0 p-0 pt-2 pb-5">
          <div class="card w-100 dashCard border-0 p-3 pt-4 pb-4">
            <div class="row m-0 p-0 mt-1">
              <div class="col-9">
                <div class="row m-0 p-0">
                  <div class="col m-0 p-0">
                    <span
                      class="material-icons text-primary mr-2 align-middle hov"
                      style="font-size: 24px"
                      @click="month -= 1"
                    >
                      chevron_left
                    </span>
                    <span class="h3 mt-auto mb-auto align-middle">
                      {{ months[month].month.toUpperCase() }} {{ year }}
                    </span>
                    <span
                      class="material-icons text-primary ml-2 align-middle hov"
                      style="font-size: 24px"
                      @click="month += 1"
                    >
                      chevron_right
                  ...
                  <div class="col-auto m-0 p-0">
                    <button
                      class="btn btn-primary boldS p-2 pl-3 pr-3 mt-auto mb-auto"
                ...
                <div class="row m-0 p-0 pt-4 pb-4">
                  <div
                    class="col m-0 p-0 text-center"
                    v-for="(week, index) in weeks"
                    :key="index"
                  >
                    <span
                      class="h3 mt-auto mb-auto boldL"
                      style="color: #717579"
                    >
                      {{ week }}
                    </span>
                ...
                <div
                  class="row m-0 p-0"
                  style="height: 97.85px"
                  v-for="row in 6"
                  :key="row"
                >
                  <div
                    :class="[
                      'col m-0 p-0  h4 border border-top-0 border-left-0',
                      {
                        'text-gray4 bg-calendarGray':
                          0 >=
                            (row - 1) * 7 +
                              (col - 1) -
                              months[month].startDay +
                              1 ||
                          (row - 1) * 7 +
                            (col - 1) -
                            months[month].startDay +
                      ...
                      { 'border-right-0': col == 7 },
                      {
                        'border-bottom-0':
                          row ==
                          Math.ceil(
                            (months[month].days + months[month].startDay) / 7
                          ),
                      },
                    ]"
                    v-for="col in 7"
                    ...
                    @click="
                      changeSelected(
                        year,
                        month,
                        (row - 1) * 7 + (col - 1) - months[month].startDay + 1,
                        col
                      )
                    "
                  >
                    <div
                      :class="[
                        'h-100 w-100 p-1',
                        {
                          'border border-darkPrimary rounded border-3':
                            month == selectedMonth &&
                            year == selectedYear &&
                            (row - 1) * 7 +
                              (col - 1) -
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          <div
                            class="bg-darkPrimary mr-1"
                            style="
                              width: 12.63px;
                              height: 12.63px;
                              border-radius: 100px;
                            "
                            v-for="index in schedules[
                              `${year}${months[month].month}${
                                (row - 1) * 7 +
                                (col - 1) -
                                months[month].startDay +
              ...
              <div
                class="col-3 p-0 border border-top-0 border-bottom-0 border-right-0"
                style="max-height: 695.591px; overflow-y: scroll"
              >
                <p class="h4 mb-0 ml-2 pl-2">Schedules</p>
                <p class="regularM text-dark4 ml-2 pl-2 mb-5">
                  {{
                    months[selectedMonth].month +
                    " " +
                    selectedDate +
                    ", " +
                    weeksful[selectedWeek]
                  }}
                </p>
                <div
                  v-if="
                    `${selectedYear}${months[selectedMonth].month}${selectedDate}` in
                    schedules
                  "
                >
                  <div
                    class="border border-bottom-0 border-left-0 border-right-0 p-2 pt-3 pb-3 mt-4"
                    v-for="(time, index) in defaultTime"
                    :key="index"
                  >
                    <div class="regularM m-0 mt-n4 p-0 ml-4">
                      <span class="pl-3 pr-3 w-auto bg-white">
                        {{ time }}
                      </span>
                    </div>
                    <div
                      v-if="
                        time in
                        schedules[
                          `${selectedYear}${months[selectedMonth].month}${selectedDate}`
                        ].todo
                      "
                    >
                      <div
                        ...
                        :key="task"
                        class="regularM p-4 bg-primary mt-3 ml-2 text-white"
                        style="border-radius: 16px"
                      >
                        {{ task }}
                      </div>
                      <div
                        ...
                        "
                        class="regularM p-4 ml-2 bg-gray mt-3"
                        style="border-radius: 16px"
                      >
                        {{
                          schedules[
...
export default {
  ...
  data() {
    return {
      ...
      months: [
        { month: "January", days: 0, startDay: 0 },
